        ...
        <div class='font-NunitoSans'>
            ...
            <main>
                <section className="bg-singleLayoutsStandardBackground bg-no-repeat text-center text-white">
                    <h1 className="font-Amiri text-6xl pt-60 mb-6">Grid 4 Columns</h1>
                    <p className="font-semibold text-sm pb-48 z-10"><span className="opacity-50">HOME &gt; </span> GRID 4 COLUMNS</p>
                </section>


                <div className='w-[98%] m-auto mb-40 mt-10'>
                    <div className='grid grid-cols-4 relative -top-4'>
                        {ArticleData.slice(0, loadMore).map((el) => {
                            return(
                                <Link key={el.index} href='/Blog/BlogSingle/Standard'>
                                <div className='articleAppear mt-4 mb-6 relative border-solid border border-[#e5e5e5] rounded-lg p-5 max-w-[440px] hover:shadow-[6px_5px_30px_0px_rgb(0,0,0,0.10)] hover:border-transparent ease-in-out duration-300 hover:cursor-pointer'>
                                    <Image  src={el.background} alt='persone travaillant sur un ordinateur' className='w-[400px] rounded-lg' />

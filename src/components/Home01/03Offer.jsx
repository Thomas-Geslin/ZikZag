        ...
        <section id='goTopViewport' className='bg-greyBackground bg-squareBackground bg-no-repeat bg-[top_right_5rem] text-center'>
            ...
            <div className='grid grid-cols-3 m-auto w-[63%] mt-20'>
                {OfferGridData.map((el) => {
                    return(
                        <div key={el.index} className={`bg-${el.background} bg-no-repeat rounded-lg w-[370px] m-auto mb-20 relative group hover:cursor-pointer hover:shadow-[6px_5px_30px_0px_rgb(0,0,0,0.15)] ease-in-out duration-500`}>
            ...
            <button className="rounded bg-orange mb-60 text-white font-semibold text-sm py-4 px-10 hover:bg-darkBlueBackground ease-in-out duration-300">ALL SERVICES</button>
        </section>
    )
}
...
export default function WidgetColor() {
    ...
    return(
            <div id='colorWidgetTarget' className='fixed left-0 top-96 z-[200] flex flex-col bg-[#fcfcfc] shadow-lg text-center rounded-br px-6 pt-8 pb-10 -translate-x-full ease-in-out duration-500'>
                ...
                <p className='font-Amiri text-2xl text-slightGrey'>Choose Color:</p>

                <span className='w-48 h-px bg-[#e5e5e5] absolute top-[70px] left-[50%] translate-x-[-50%]'></span>

                <div className='flex justify-between mt-6'>
                    <button onClick={() => { setColor('orange'); setSecondaryColor('darkBlueBackground'); }} className='font-bold text-sm bg-orange py-4 px-10 rounded text-white mr-5 hover:bg-darkBlueBackground ease-in-out duration-300'>CLASSIC</button>
                    <button onClick={() => { setColor('red'); setSecondaryColor('secondaryBlue'); }} className="font-bold text-sm bg-[#ec3b3b] py-4 px-9 rounded text-white hover:bg-[#2e386b] ease-in-out duration-300">OTHER</button>
                </div>
            </div>
    )
}
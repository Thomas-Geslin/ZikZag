...
export default function ConsultancyFAQ() {
    ...
    return(
        <section className='mt-80 relative bg-greyBackground -mx-[33.3%] -mb-40'>
            <div className='bg-transparent relative -top-48 mb-28 w-10/12 m-auto rounded-xl relative text-center max-xl:flex max-xl:flex-col-reverse max-xl:items-center max-xl:w-full'>
            ...
            <div className='text-center mx-[19%] relative -top-52'>
                <h2 className='text-orange font-bold mb-4 w-48 m-auto relative max-xl:pt-36 850:mt-12'><span className='absolute bottom-1 left-3.5 bg-orange h-0.5 w-2'></span>SUPPORT CENTER<span className='absolute bottom-1 ml-1 bg-orange h-0.5 w-2'></span></h2>
                <h3 className='font-Amiri text-slightGrey text-5xl mb-5'>Still Need help?</h3>
                <p className='text-greyText text-lg w-[65%] m-auto mb-20 1060:w-9/12'>We are specialists in both economics and information technologies and we apply our full range of talent to creating the perfect solution for each client’s needs.</p>
            
                <div className='flex'>
                    {FaqCardElement.map((el) => {
                        return(
                            <div key={el.index} className={`FaqCardBackground-${el.index} rounded-lg w-[370px] m-auto mb-8 relative group hover:cursor-pointer hover:shadow-[6px_5px_30px_0px_rgb(0,0,0,0.15)] ease-in-out duration-500`}>
